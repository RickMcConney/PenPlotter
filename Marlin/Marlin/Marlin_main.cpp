...
#ifdef POLAR
float polarHomeY = 250;
float polarPageWidth = 840;
float polarLastA;
float polarLastB;



void polarHome(float homeY)
{
  polarHomeY = homeY;
  float x = polarPageWidth/2;
  float y = polarHomeY;
  current_position[X_AXIS] = x;
  current_position[Y_AXIS] = y;
  
  float aPos = sqrt(x*x+y*y);
  float bPos = sqrt(sq(polarPageWidth-x)+y*y);
  long a = lround(aPos*axis_steps_per_unit[X_AXIS]);
  long b  = lround(bPos*axis_steps_per_unit[Y_AXIS]);
    
  st_set_position(a,b,0,0);
  polarLastA = aPos;
  polarLastB = bPos;
  
}

void polarWidth(float value)
{
  polarPageWidth = value;
  polarHome(polarHomeY);
}

void polarSegmentLine(float x2, float y2)
{
...
void process_commands()
{
    ...
    case 1: // G1
      if(Stopped == false) {
        get_coordinates(); // For X Y Z E F
          ...
          #endif //FWRETRACT
          #ifdef POLAR
            polarSegmentLine(destination[X_AXIS],destination[Y_AXIS]);
          #else
            prepare_move();
          #endif
  ...
  else if(code_seen('M'))
  {
   ...
   {
     if (code_seen('Y')) {
         polarHome(code_value()); 
      }
   }
   case 4:
   {
     if(code_seen('X')){
       polarWidth(code_value());
     }
   }
#endif
#ifdef ULTIPANEL
...
#if ! (defined DELTA || defined SCARA || defined POLAR)
