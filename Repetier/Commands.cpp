...
#else
                if(com->hasX())
                  Printer::currentPosition[X_AXIS] = com->X;
                if(com->hasY())
                  Printer::currentPosition[Y_AXIS] = com->Y;
                PrintLine::segmentLine();
               // PrintLine::queueCartesianMove(ALWAYS_CHECK_ENDSTOPS,true);
#endif
            break;
#if ARC_SUPPORT
          ...
          case 2: //M2      
              if (com->hasS() && com->hasP())
              {
                ...
                int P = com->P;
                 ...
                 pixel_drawSquarePixel(S,P,mSE,1); 
                 pixel_drawSquarePixel(S,P,mSE,1); 
                 pixel_drawSquarePixel(S,P,mNE,1); 
                 pixel_drawSquarePixel(S,P,mNE,1); 
                 pixel_drawSquarePixel(S,P,mSE,1); 
                 pixel_drawSquarePixel(S,P,mSE,1);
             
                 pixel_drawSquarePixel(S,P,mSW,1); 
                 pixel_drawSquarePixel(S,P,mSW,1); 
                 pixel_drawSquarePixel(S,P,mSE,1); 
                 pixel_drawSquarePixel(S,P,mSE,1); 
                 pixel_drawSquarePixel(S,P,mSW,1); 
                 ...
                 pixel_drawSquarePixel(S,P,mNW,1); 
                 pixel_drawSquarePixel(S,P,mNW,1); 
                 pixel_drawSquarePixel(S,P,mSW,1); 
                 pixel_drawSquarePixel(S,P,mSW,1); 
                 pixel_drawSquarePixel(S,P,mNW,1); 
        ...
        case 3: //M3           
            if(com->hasX() && com->hasY() &&
                  com->hasE() && com->hasS() && com->hasP())
               {
                 ...
                 if(da >0){
                   PrintLine::moveSteps(da/2,0,true); 
                   PrintLine::moveSteps(0,db,true);
                   PrintLine::moveSteps(da/2,0,true);   
                   pixel_drawSquarePixel(size,shade,dir,0);                
                 }
                 else
                 {
                    if(db > size)
                    {
                      ...
                      PrintLine::moveSteps(0,db-size,true);
                      PrintLine::moveSteps(-size/2,0,true);
                    }
                    else if(db < -size)
                    {
                    ...
                    pixel_drawSquarePixel(size,shade,dir,1); 
                 }  
               }                 
