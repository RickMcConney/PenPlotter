...
float pageWidth = 840;
float mmPerRev = 20;
float stepPerRev = 1600;
float toStep = stepPerRev/mmPerRev;
float toMM = mmPerRev/stepPerRev;
...
void setMmPerRev(float mm)
{
  mmPerRev = mm;
  toStep = stepPerRev/mmPerRev;
  toMM = mmPerRev/stepPerRev;
}

void setStepsPerRev(float steps)
{
  stepPerRev = steps;
  toStep = stepPerRev/mmPerRev;
  toMM = mmPerRev/stepPerRev; 
...
{
  return sqrt(sq((pageWidth-cX))+cY*cY);
}

float mmToStep(float x)
{
    return x*toStep;
}

float stepTomm(float x)
{
  return x*toMM;
}
...
void PrintLine::home(float homeY)
{
   Printer::currentPosition[X_AXIS] = pageWidth/2;
   Printer::currentPosition[Y_AXIS] = homeY;
   Printer::lastCmdPos[X_AXIS] = Printer::currentPosition[X_AXIS];
   Printer::lastCmdPos[Y_AXIS] = Printer::currentPosition[Y_AXIS];
   
   float x = Printer::currentPosition[X_AXIS];
   float y = Printer::currentPosition[Y_AXIS];
   long tA = round(mmToStep(getMachineA(x,y)));
   long tB = round(mmToStep(getMachineB(x,y)));
   Printer::currentPositionSteps[X_AXIS] = -tA;
   Printer::currentPositionSteps[Y_AXIS] = tB;
}

void PrintLine::segmentLine()
{
      float x1 = Printer::lastCmdPos[X_AXIS];
      float y1 =  Printer::lastCmdPos[Y_AXIS];
      float x2 =  Printer::currentPosition[X_AXIS];
      float y2 =  Printer::currentPosition[Y_AXIS];
      float dx = x2-x1;
      float dy = y2-y1;
      float x = x1;
      float y = y1;
     ...
     // Com::print("\n ");
      for(int s = 0;s<steps-1;s++)
      {
        x+=dx;
        y+=dy;
        Printer::currentPosition[X_AXIS] = x;
        Printer::currentPosition[Y_AXIS] = y;

        queueCartesianMove(ALWAYS_CHECK_ENDSTOPS,true);
      }
      Printer::currentPosition[X_AXIS] = x2;
      Printer::currentPosition[Y_AXIS] = y2;
      queueCartesianMove(ALWAYS_CHECK_ENDSTOPS,true);
      Printer::lastCmdPos[X_AXIS] = x2;
      Printer::lastCmdPos[Y_AXIS] = y2;
...
void PrintLine::moveRawDeltaSteps(long da, long db,uint8_t pathOptimize)
{
  // Com::print("move steps ");
   ...
   long tA = Printer::currentPositionSteps[X_AXIS] - da;
   long tB = Printer::currentPositionSteps[Y_AXIS] + db;
   Printer::destinationSteps[X_AXIS] = tA;
   Printer::destinationSteps[Y_AXIS] = tB;
  // Com::print("tA ");
  // Com::print((int)tA);
  // Com::print("tB ");
  // Com::print((int)tB); 
   float aPos = stepTomm(tA);
   float bPos = stepTomm(tB);
 //  Com::print("aPos ");
 //  Com::print((int)aPos);
 //  Com::print("bPos ");
 //  Com::print((int)bPos);
   float cx = getCartesianX(aPos,bPos);
   Printer::currentPosition[X_AXIS] = cx;
   float cy = getCartesianY(cx,aPos);
   Printer::currentPosition[Y_AXIS] = cy;
   Printer::lastCmdPos[X_AXIS] = cx;
   Printer::lastCmdPos[Y_AXIS] = cy;
  ...
  // Com::print("dx ",Printer::destinationSteps[X_AXIS]);  
  // Com::print("dy ",Printer::destinationSteps[Y_AXIS]); 
...
void PrintLine::moveSteps(float a, float b,uint8_t pathOptimize)
{
  // Com::print("move steps ");
   ...
   float tA = Printer::currentPositionSteps[X_AXIS] - mmToStep(a);
   float tB = Printer::currentPositionSteps[Y_AXIS] + mmToStep(b);
   Printer::destinationSteps[X_AXIS] = round(tA);
   Printer::destinationSteps[Y_AXIS] = round(tB);
  // Com::print("tA ");
  // Com::print((int)tA);
  // Com::print("tB ");
  // Com::print((int)tB); 
...
void PrintLine::calculateDirectionAndDelta(long difference[], flag8_t *dir, long delta[])
{
    *dir = 0;
    //Find direction
    for(uint8_t i=0; i < 4; i++)
...
#endif
        float sps = static_cast<float>((cartesianDir & 136)==136 ? Printer::printMovesPerSecond : Printer::travelMovesPerSecond);
...
long PrintLine::bresenhamStep() // Version for delta printer
{
    ...
    if(cur->stepsRemaining>0)
    {
        ...
        if(doOdd)
        {
            ...
            {
                /*  if(firstFull) {
                      Com::printF(PSTR("VMR:"),(long)Printer::vMaxReached);
                      Com::printF(PSTR(" VM:"),(long)cur->vMax);
                      Com::printFLN(PSTR(" AS:"),(long)cur->accelSteps);
                  }*/
                firstFull = false;
